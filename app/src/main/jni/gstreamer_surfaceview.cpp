...
static JNIEnv *attach_current_thread(void) {
    ...
    args.group = NULL;
    if (!java_vm)
        GST_DEBUG("java_vm not set");
    if (java_vm->AttachCurrentThread(&env, &args) < 0) {
        GST_ERROR("Failed to attach current thread");
        return NULL;
    }
    GST_DEBUG("Attaching thread... OK");
    return env;
}
...
static void gst_native_set_pipeline(JNIEnv *env, jobject thiz, jstring str) {
    jboolean isCopy;
...
static void gst_native_init(JNIEnv* env, jobject thiz) {
    ...
    GST_DEBUG_CATEGORY_INIT(debug_category, "GStreamerSurfaceView", 0, "GStreamerSurfaceView");
    gst_debug_set_threshold_for_name("GStreamerSurfaceView", GST_LEVEL_DEBUG);
    GST_DEBUG("Created CustomData at %p", data);
    data->app = env->NewGlobalRef(thiz);
    GST_DEBUG("Created GlobalRef for app object at %p", data->app);
    __android_log_print(ANDROID_LOG_INFO, "GStreamerSurfaceView", "gst_native_init Created CustomData at %p", data);
}

static void gst_native_start(JNIEnv* env, jobject thiz) {
    CustomData *data = GET_CUSTOM_DATA(env, thiz, custom_data_field_id);
    __android_log_print(ANDROID_LOG_INFO, "GStreamerSurfaceView", "gst_native_start Created CustomData at %p", data);
    if (!data)
        return;
    pthread_create(&gst_app_thread, NULL, &app_function, data);
    __android_log_print(ANDROID_LOG_INFO, "GStreamerSurfaceView", "gst_native_start OK");
}
...
static jboolean gst_native_class_init(JNIEnv* env, jclass klass) {
    ...
    if (!custom_data_field_id) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "The calling class does not implement native_custom_data");
        return JNI_FALSE;
    }
    if (!set_message_method_id) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "The calling class does not implement setMessage");
        return JNI_FALSE;
    }
    if (!notify_state_method_id) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "The calling class does not implement notifyState");
        return JNI_FALSE;
    }
    if (!on_gstreamer_initialized_method_id) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "The calling class does not implement onGStreamerInitialized");
        return JNI_FALSE;
    }
    if (!set_error_method_id) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "The calling class does not implement setError");
        return JNI_FALSE;
    }

    __android_log_print(ANDROID_LOG_INFO, "GStreamerSurfaceView", "gst_native_class_init OK");
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if (vm->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, "GStreamerSurfaceView", "Could not retrieve JNIEnv");
        return 0;
    }
    jclass klass = env->FindClass("org/wilds/gstreamer/GStreamerSurfaceView");
    env->RegisterNatives(klass, native_methods, G_N_ELEMENTS(native_methods));

    pthread_key_create(&current_jni_env, detach_current_thread);
    char *version_utf8 = gst_version_string();
    __android_log_print(ANDROID_LOG_VERBOSE, "GStreamerSurfaceView", "GSTREAMER VERSION: %s", version_utf8);
    g_free(version_utf8);
